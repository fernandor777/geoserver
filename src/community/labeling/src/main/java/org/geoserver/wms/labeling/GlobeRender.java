...
public class GlobeRender {
    ...
    static final int BOTTON_TRIANGLE_WIDTH = 15;
    static final int BOTTON_TRIANGLE_HEIGHT = 20;
    
    static final AlphaComposite SHADOW_COMPOSITE = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, SHADOW_ALPHA);
    ...
    static void renderGlobe(Graphics2D graphics2d, GlobeBounds bounds, GlobeRenderConfiguration config) {
        renderShadow(graphics2d, bounds, config);
        renderGlobeFront(graphics2d, bounds, config);
    }
    
    private static void renderGlobeFront(Graphics2D graphics2d, GlobeBounds bounds, GlobeRenderConfiguration config) {
    ...
    private static Area buildGlobeShape(GlobeBounds bounds, double delta, GlobeRenderConfiguration config) {
        RoundRectangle2D rectangle = new RoundRectangle2D.Double(
                delta, delta, 
                bounds.getWidth(), bounds.getHeight(), bounds.getRadius(), bounds.getRadius());
    ...
    private static void renderShadow(Graphics2D graphics2d, GlobeBounds bounds, GlobeRenderConfiguration config) {
    ...
    private static Shape getGlobeBottonTriangle(GlobeBounds bounds, double delta, 
            GlobeRenderConfiguration config) {
        ...
        Polygon triangle = new Polygon(new int[] { (int) xmin, (int) xmax, (int) middlex }, 
                new int[] { (int) ymin, (int) ymin, (int) ymax }, 
                3);
        return triangle;
    }
    
    public static class GlobeRenderConfiguration {
        private int tailWidth;
        private int taildHeight;
        private int tailWidthHalf;
        
        public GlobeRenderConfiguration(int tailWidth, int taildHeight) {
            super();
        ...
        public String toString() {
            return "GlobeRenderConfiguration [tailWidth=" + tailWidth + ", taildHeight=" + taildHeight + "]";
        }
    }
    
    static GlobeRenderConfiguration defaultRenderConfig() {
        return new GlobeRenderConfiguration(BOTTON_TRIANGLE_WIDTH, BOTTON_TRIANGLE_HEIGHT);
    }
}
