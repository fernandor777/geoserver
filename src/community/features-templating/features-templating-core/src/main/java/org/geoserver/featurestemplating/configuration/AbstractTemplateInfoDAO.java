...
/** A template info DAO that uses a property file for persistence. */
public abstract class AbstractTemplateInfoDAO implements TemplateInfoDAO {
    ...
    public AbstractTemplateInfoDAO(GeoServerDataDirectory dd, String propertyFileName) {
        this.dd = dd;
        this.propertyFileName = Objects.requireNonNull(propertyFileName);
        Resource templateDir = dd.get(TEMPLATE_DIR);
        File dir = templateDir.dir();
        if (!dir.exists()) dir.mkdir();
        Resource prop = dd.get(TEMPLATE_DIR, propertyFileName);
        prop.file();
        ...
        catalog.addListener(new TemplateInfoDAOImpl.CatalogListenerTemplateInfo());
    }
    ...
    public void storeProperties() {
        ...
        Resource propFile = dd.get(TEMPLATE_DIR, propertyFileName);
        try (OutputStream os = propFile.out()) {
            p.store(os, null);
        } catch (Exception e) {
            throw new RuntimeException("Could not write rules to " + propertyFileName);
        }
    }
