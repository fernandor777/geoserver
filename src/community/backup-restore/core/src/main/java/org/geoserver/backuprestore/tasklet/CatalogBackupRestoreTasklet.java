...
public class CatalogBackupRestoreTasklet extends AbstractCatalogBackupRestoreTasklet {
    ...
    private static final String GWC_GEO_SERVERV_CONFIG_PERSISTER = "gwcGeoServervConfigPersister";
    ...
    private void backupFilteredGwc(
        ...
        if (!skipGWC) {
            try {
                if (GeoServerExtensions.bean(GWC_GEO_SERVERV_CONFIG_PERSISTER) != null) {
    ...
    private void backupGWCLayers(Resource targetBackupFolder) throws Exception {
        GWCConfigPersister gwcGeoServerConfigPersister =
                (GWCConfigPersister) GeoServerExtensions.bean(GWC_GEO_SERVERV_CONFIG_PERSISTER);
